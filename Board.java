...
public class Board {
	...
	public void checkBoardSize(int rows, int cols) {
		if ( (rows <= 0) || (cols <= 0) || (rows % 2 == 1) || (cols % 2 == 1) || (rows*cols > 52) ) {
			throw new IllegalArgumentException("The size of the board is not valid. 0 < board size <= 52 and (rows and cols must be positive even integers)");
		}
	}
	...
	public void initBoard(int rows, int cols) {
		...
		for (int i=0; i<rows; i++) {
			for (int j=0; j<cols; j++) {
				this.board[i][j] = new Cell(symbols.get(i*cols+j));
			}
		}
	}
	...
	public boolean isCellValid(Position pos) {
		if ( (pos.getRow() < 0) || (pos.getRow() >= board.length) || (pos.getCol() < 0) || (pos.getCol() >= board[0].length) )
			return false;
		return true;
	}
